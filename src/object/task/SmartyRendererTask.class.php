...
//        $this->smarty->default_modifiers  = $config->getArray( 'default_modifiers', array() )->unbox();
     ...
     *
     * $charcoal.profile.XXX       --- defined values in profile.ini
     * $charcoal.cookie.XXX        --- defined values in server cookies
     * $charcoal.session.XXX       --- defined values in server session
     * $charcoal.request.id        --- request ID(sha1 hash)
     * $charcoal.request.path      --- request object path(i.e. @:login:form)
     * $charcoal._smarty           --- Smarty object
     *
     * @param Charcoal_IEventContext $context   event context
     */
    protected function renderLayout( $context )
    {
        $charcoal = array();
    ...
    public function processEvent( $context )
    {
        ...
        // render layout
        if ( $event instanceof Charcoal_RenderLayoutEvent )
        {
            set_error_handler( array($this,"onUnhandledError") );
            try{
